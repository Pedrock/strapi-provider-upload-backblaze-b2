  ...
  for (let i = 0; i < 5; i++) {
    ...
    try {
      ...
      return data;
        ...
        if (status === 401 /* Unauthorized */ && response.data &&
          (response.data.code === 'expired_auth_token' || response.data.code === 'bad_auth_token')) {
            // Upload auth token has expired.  Time for a new one.
            strapi.log.debug("Backblaze B2 Upload auth token has expired.")
            urlAndAuthToken = null;
        }
